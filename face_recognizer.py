...
class Learner(QRunnable):
    ...
    def run(self):
        ...
        faces, labels = get_labels_faces(users)
...
class Recognizer(QRunnable):
    ...
    def run(self):
        camera = Camera()
        has_found = False
        while has_found == False and self.__recognize == True:
...
class FaceRecognizerManager():
    def __init__(self, thread_pool, is_learning_callback ,finished_learning_signal, recognized_user_signal): 
        self.__finished_learning_signal = finished_learning_signal
        self.__finished_learning_signal.connect(finished_learning)
        self.__recognized_user_signal = recognized_user_signal
    ...
    def learn(self): # We need a signal/slot to inform when recognizer is shut down -> in this slot set recognizer to None
        self.__queue.put(Learner(self.__finished_learning_signal)) 
    ...
    def schedule(self):
        if not self.__learn_queue.empty():
            self.__is_learning_callback()
            self.__thread_pool.start(self.__queue.get())     
        else:
...
# 3. learn triggered by API during show widgets 
# 4. triggered learning during learning 
